...
foreach (var grupoProduto in itens.GroupBy(item => item.Gtin))
{
    var produtoInfo = produtos.FirstOrDefault(p => p.Gtin == grupoProduto.Key);
    if (produtoInfo == null) continue;

    int totalUnidades = grupoProduto.Sum(item => (int)item.Quantidade);

    if (produtoInfo.PrecoAtacado.HasValue && 
        produtoInfo.UnidadesAtacado.HasValue && 
        totalUnidades >= produtoInfo.UnidadesAtacado.Value)
    {
        decimal valorDesconto = (produtoInfo.PrecoVarejo - produtoInfo.PrecoAtacado.Value) * totalUnidades;
        
        descontos.Add(new DescontoItem
        {
            Gtin = grupoProduto.Key,
...
foreach (var desconto in descontos)
    Console.WriteLine("{0}   {1:C2}",desconto.Gtin,desconto.Desconto);
...
Console.WriteLine("Descontos =   {0:C2}",totalDescontos);
